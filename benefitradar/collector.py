...
def _collect_browser_pass(
    ...
    *,
    ...
    from radar_core.browser_collector import collect_browser_sources
...
def collect_sources(
    ...
    *,
    ...
    standard_sources = [
        source for source in enabled_sources if source.type.lower() in {"rss", "api", "bokjiro"}
    ]
    browser_sources = [
        source
    ...
    unsupported_sources = [
        source for source in enabled_sources if source.type.lower() not in supported_types
    ]
            ...
            if standard_sources:
                with ThreadPoolExecutor(max_workers=workers) as executor:
                    future_map: list[Future[tuple[list[Article], list[str]]]] = [
                        executor.submit(_collect_for_source, source) for source in standard_sources
                    ]
