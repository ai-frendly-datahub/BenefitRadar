...
def handle_benefit_match(*, db_path: Path, query: str = "", days: int = 30, limit: int = 10) -> str:
    """Match benefits or subsidies from recent articles based on query."""
    cutoff = datetime.now() - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        if query.strip():
    ...
    lines = [f"Matched {len(rows)} benefit(s):"]
    for title, source, link, entities_json, collected_at in rows:
        ...
        if entities_json:
            try:
                ...
                if entities:
                    entity_tags = f" [{', '.join(entities.keys())}]"
            except json.JSONDecodeError:
                pass
